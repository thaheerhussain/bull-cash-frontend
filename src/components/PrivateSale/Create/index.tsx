      ...
      if (account) {
        if (chainId) {
          ...
          let para = [
            [account, account],
            [
              mulDecimal("1", 18).toString(), // 0
              mulDecimal(value.minimumBuy, 18).toString(), // 1
              mulDecimal(value.maximumBuy, 18).toString(), // 2
              mulDecimal(value.softcap, 18).toString(), // 3
              mulDecimal(value.hardcap || "0", 18).toString(), // 4
              String(Math.floor(new Date(value.startTime).getTime() / 1000.0)), // 5
              String(Math.floor(new Date(value.endTime).getTime() / 1000.0)), // 6
              feesSetting[
                value.feeOptions ===
              ...
              "2", // 9
              "2", // 10
              (value.saleMethod === "Whitelist" ? 1 : 0).toString(), // 11
              "0", // 12
            ], //2
            JSON.stringify({
              logo: logo,
          ...
          const params: LibPresale.PrivateSaleStruct = {
            ...
            tier1: {
              endTime: Math.floor(
                new Date(value.tier1EndTime || 0).getTime() / 1000.0
              ),
            },
            tier2: {
              endTime: Math.floor(
                new Date(value.tier2EndTime || 0).getTime() / 1000.0
              ),
            },
          ...
          if (response != null) {
            if (response.status) {
              ...
              if (typeof response.logs[2] !== "undefined") {
                await router.push(
                  `/private-sale/details/${response.logs[2].address}`
                );
              } else {
              ...
              <h1 className="text-4xl md:text-6xl 2xl:text-7xl text-primary-green mt-4">
                {formik.values.title ? (
                  <div className="flex lg:max-w-[90%]">
                    ...
                    <div className="min-w-fit ">-Privatesale</div>
                  </div>
                ) : (
                  "Create Private Sale"
                )}
